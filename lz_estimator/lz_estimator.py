...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Give dataset & output image name with FORMAT as a command line argument")
        exit()
    ...
    dataset = f.read()
    entropies_list = []
    for i in range(2,len(dataset),1000):
        entropies_list.append(lz_complexity(dataset[:i])/(i+0.0)*np.log(i))

    plt.semilogx(range(2, len(dataset), 1000), entropies_list)
